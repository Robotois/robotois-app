...
CustomDialog.show = function (title, message, options) {
  ...
  if (!dialogDiv) {
    ...
    dialogDiv.style.cssText =
      'background-color: #fff;' + 'width: 400px;' + 'margin: 20px auto 0;' + 'padding: 10px;';
  ...
  dialogDiv.innerHTML = `${'<header class="customDialogTitle"></header>' +
    '<p class="customDialogMessage"></p>'}${options.showInput
    ? '<div><input id="customDialogInput"></div>'
    : ''}<div class="customDialogButtons">${options.showCancel
    ? '<button id="customDialogCancel">Cancel</button>'
    : ''}${options.showOkay ? '<button id="customDialogOkay">OK</button>' : ''}</div>`;
  dialogDiv
